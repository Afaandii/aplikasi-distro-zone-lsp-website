...
export default function EditTipe() {
  const { id_tipe } = useParams<{ id_tipe: string }>();
  ...
  const fetchTipe = async () => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:8080/api/v1/tipe/${id_tipe}`,
        {
    ...
    <>
      ...
      <section className="mb-6">
        <div className="flex items-center justify-between p-3 rounded-t-lg">
          <h1 className="text-2xl font-bold text-white">Form Edit Tipe</h1>
        </div>
      </section>
